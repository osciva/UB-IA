...
class Chess():
    ...
    def __init__(self, initboard, myinit=True):
        ...
        #Donada una situació, creem un tauler amb les peces a les posicions de la situació
        if myinit:
            self.board = board.Board(initboard,False)
            self.boardSim = board.Board(initboard,False)
        #Si no creem un tauler nou, amb totes les peces en la posició inicial del joc
        else:
            self.board = board.Board([],True)
            self.boardSim = board.Board([],True)

        #Iniciem amb el torn de les blanques
        self.turn = True

        #Inicialment no hi ha cap peça matada
    ...
    # Metode per canviar peo per una peça al arribar al final oposat del tauler.
    # No haurien de ser només peces mortes(???) Utilitzem ghost_piece
    ...
    def moveSim(self, start, to):
        ...
        #Comprova que hi ha hagi una peça per moure a la casilla inicial
        if self.boardSim.board[start[0]][start[1]] == None:
            print("There is no piece to move at the start place")
            return

        #Comprova que no hi hagi cap peça a la destinació(?)
        if self.boardSim.board[to[0]][to[1]] != None:
            print("other piece there")

        target_piece = self.boardSim.board[start[0]][start[1]] #Peça que movem
        ...
        end_piece = self.boardSim.board[to[0]][to[1]] #Peça de la pos desti
        ...
        if target_piece.is_valid_move(self.boardSim, start, to):
            ...
            # Special check for if the move is castling (comprova si estem fent un enrocament)
            # Board reconfiguration is handled in Piece
            if target_piece.name == 'K' and abs(start[1] - to[1]) == 2:

                print("castled")

                #No entiendo que hace aqui:') Després de l'enrocament, treu les fitxes fantasmes del en passant(?)
            ...
            #Si hi ha una peça a la posicio destí vol dir que es del color contrari, per tant ens la menjem
    ...
    def move(self, start, to):
        ...
        precondition: `start` and `to` are valid positions on the board
        """
        #Comprova que hi hagi una peça a la pos inicial
        if self.board.board[start[0]][start[1]] == None:
            print("There is no piece to move at the start place")
            return

        #Hagafem la peça que volem moure
        ...
        #Mirem si hi ha una peça a la pos destí
        ...
        #Si el moviment que volem fer és vàlid
        if target_piece.is_valid_move(self.board, start, to):
            ...
            #Si hi ha una peça a la pos destí és del color contrari
...
#  chess = Chess([],False)
    ...
    while True:
        ...
        i = 0
        while i < 8:
            #Comprovem si es pot aplicar promotion a algún peon
